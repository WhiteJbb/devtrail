...
관련 문서:
- [vault-consumption-plan.md](./vault-consumption-plan.md)
- [fable_check.md](./fable_check.md)
...
| 학습 회수 | `suggest-knowledge`는 있지만 "AI가 대신 처리해서 내가 놓친 부분"을 추적하지 않음 | `capture-session` 스키마에 Learning Recovery 섹션 추가 |
...
| 구분 | 유지/노출 대상 | 이유 |
|---|---|---|
| MCP tool | `get_project_briefing`, `search_vault`, `read_note`, `build_context`, `record_note`, `write_work_plan`, `write_session_process`, `record_agent_improvement`, `list_open_loops`, `suggest_next_actions` | 에이전트가 세션 중 자동으로 호출해야 하는 읽기/후보 기록/인계 도구 |
...
- 현재 repo root, git remote, 디렉터리 이름으로 project 후보를 추론한다.
...
- `60_Candidates/SessionHandoffs/<Project>/`의 최신 Plan/Process N개는 검색 점수와 무관하게 반드시 포함한다.
- 결과를 "오늘 작업에 바로 필요한 맥락" 중심으로 요약한다.
...
- 원본 세션 기록은 기존 `capture-session` 흐름을 사용한다.
- Plan/Process는 `60_Candidates/SessionHandoffs/<Project>/`에 candidate로 남긴다.
- Plan/Process candidate는 `candidate_type: session_handoff`, `handoff_type: plan | process`, `project`, `source_refs`, `status: candidate`를 frontmatter에 포함한다.
...
- `SessionHandoffs`는 promote 대상이라기보다 다음 세션 briefing의 운영 메모리다. 오래된 handoff는 digest/cleanup 정책으로 관리하고, 공식 지식으로 남길 내용만 Decisions/MemoryPatches/docs 후보로 분리한다.
...
`vault-consumption-plan.md`의 방향은 유지하되, `60_Candidates/` read_scope 설명 충돌을 먼저 정리한다.

정리할 기준:

- `60_Candidates/`는 read_scope에 포함한다.
- 검색 결과에는 `status=candidate`를 붙인다.
- candidate는 확정 지식이 아니라 "초안/후보"로 인용하도록 MCP/tool 설명에 명시한다.
- read_scope 밖 예시는 `00_Inbox/`, `10_Worklog/`로 쓴다.

완료 기준:

- 문서 안에서 `60_Candidates/`가 "읽기 허용 후보 영역"으로 일관되게 설명된다.
- 구현자가 문서를 읽고 read/write scope를 오해할 여지가 없다.
...
| `write_session_process(...)` | 세션 종료 Process를 `session_handoff` candidate로 기록 |
...
- 실제 사용 중인 agent instruction 문서에 "세션 시작 시 `get_project_briefing`을 먼저 호출한다"는 지침을 추가한다.
- 작업 시작 전 `write_work_plan`, 컴팩팅 전 또는 세션 종료 시 `write_session_process`를 호출한다는 지침을 추가한다.
- MCP가 연결되지 않은 경우에는 기존 브리핑 파일 또는 브리핑 CLI 출력 같은 fallback 경로를 안내한다.

### P4. Learning Recovery 스키마 추가

새 커맨드를 만들기 전에 `capture-session --from-agent` 요약 스키마에 다음 섹션을 추가한다.
...
- `capture-session --from-agent --summary-file ...` 작성 규칙에 Learning Recovery가 포함된다.
...
Learning Recovery가 쌓이기 시작하면 `nightly-distill` 또는 `push-digest --daily`에 하루 1개의 복습 질문을 포함한다.
...
1. `vault-consumption-plan.md`의 `60_Candidates/` read_scope 표현을 정리한다.
2. Agent Session Lifecycle 기준으로 tool 목록과 호출 시점을 확정한다.
3. `feat/vault-mcp` 브랜치에서 `app/vault_tools.py`와 테스트를 구현한다.
4. `mcp-serve`와 MCP 서버를 추가해 Claude Code/Desktop에서 시작 브리핑·작업 전 Plan 작성·작업 중 기록·Process 작성 왕복을 검증한다.
5. agent instruction 문서에 세션 시작 `get_project_briefing`, 작업 시작 전 `write_work_plan`, 컴팩팅 전/세션 종료 `write_session_process` 지침을 추가한다.
6. `capture-session` 스키마에 Learning Recovery 섹션을 추가한다.
7. `push-digest --daily`에 복습 질문 1개를 포함한다.
8. Telegram/ask의 `ask-vault` 흐름을 붙인다.
9. 실제 루프가 검증된 뒤 README/블로그/포트폴리오 서사를 재작성한다.
