...
### 7.6 Source Refs를 Obsidian 위키링크로 렌더링

- **문제**: `CandidateWriter._render_body`(candidate_writer.py:157-158)가 source_refs를
  `- 60_Candidates/.../x.md` 플레인 텍스트 경로로만 렌더한다. 그래서 write_session_process가
  만드는 Process → Decision/MemoryPatch 역참조 체인이 데이터로는 존재하지만 Obsidian
  그래프·백링크에 전혀 잡히지 않는다 — 세션 라이프사이클 산출물이 vault에서 고립된
  노드가 된다. (기존 distill/career_bullet 계열은 `[[stem|제목]]` 링크를 잘 만든다 —
  distill_agent.py:164,200 참고.)
- **수정**: `_render_body`의 Source Refs 렌더링을 `.md` 경로인 ref에 한해
  `- [[{Path(ref).stem}]] ({ref})` 형식으로 변경 (career_bullet_agent.py:113과 같은 관례).
  `git:abc123` 같은 비파일 ref는 기존 플레인 텍스트 유지. 한 함수 수정으로 모든
  candidate kind에 일괄 적용된다.
- **주의**: `[[stem]]`은 파일명 stem으로 해석되므로 vault 내 동명 파일이 있으면 오귀속될
  수 있다 — 괄호의 전체 경로를 함께 남기는 이유이며, 링크는 탐색 보조로만 쓴다.
- **테스트**: `test_candidate_writer.py`에 source_refs=[".md 경로", "git:abc"]로 write 후
  본문에 `[[stem]] (경로)`와 플레인 `git:abc`가 각각 있는지 검증.
