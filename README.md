...
볼트 안 `.md` 노트를 `suggest-topics` / `write-draft` 등의 입력 소스로 사용합니다. API·키·네트워크 없이 파일 read만으로 동작합니다.

### 7-1. 설정

```env
# 볼트 폴더 절대경로. 비우면 Obsidian 비활성.
OBSIDIAN_VAULT_DIR=C:/Users/User/Documents/MyVault

# 읽을 노트를 태그로 필터링(쉼표 구분). 비우면 볼트 전체 노트를 읽음.
OBSIDIAN_TAGS=blog-idea,worklog

# 읽을 노트를 폴더로 필터링(볼트 기준 상대경로, 쉼표 구분). 비우면 볼트 전체 탐색.
OBSIDIAN_FOLDERS=
```

- `OBSIDIAN_TAGS`: frontmatter `tags:` 또는 본문 인라인 `#tag`가 일치하는 노트만 수집
- `OBSIDIAN_FOLDERS`: 특정 폴더만 탐색하고 싶을 때 (예: `Blog,Daily`)
- `[[위키링크]]`는 링크 텍스트만 남기고 자동 변환됨

### 7-2. 초안을 볼트 안에서 편집하기

`.env`의 `WORKSPACE_DIR`을 볼트 안 폴더로 지정하면 생성된 초안이 Obsidian에 바로 나타납니다.

```env
WORKSPACE_DIR=C:/Users/User/Documents/MyVault/work-agent
```

이렇게 하면 `write-draft`로 만든 초안이 Obsidian에서 바로 편집 가능하고, frontmatter의 `status` 필드가 Obsidian 속성으로 표시됩니다.
