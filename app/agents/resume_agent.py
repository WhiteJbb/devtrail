"""Resume Agent — 이력서 bullet / 자기소개서 초안 (Vault + 전체 프로젝트 기반)."""
...
class ResumeAgent:
    """40_AgentMemory + 30_Projects 전체를 읽어 이력서/자기소개서 초안을 생성한다."""
    ...
    def generate(self, save: bool = True) -> ResumeResult:
        source_refs: list[str] = []
        parts: list[str] = []

        # 1. Agent Memory (CareerContext, Profile 등)
        ...
        prompt = render_prompt("resume", CONTEXT_PACK=context)
        text = self._llm().complete(prompt)

        today = (self.now or datetime.now()).strftime("%Y%m%d")
        rel_path = f"50_Outputs/Resume/{today}-resume.md"
        path = self.vault_dir / rel_path

        if save:
            path.parent.mkdir(parents=True, exist_ok=True)
            path.write_text(text + "\n", encoding="utf-8")
            self.wiki_service.append_vault_log("resume", "이력서 초안", [rel_path])

        return ResumeResult(text=text, path=path, source_refs=source_refs)
